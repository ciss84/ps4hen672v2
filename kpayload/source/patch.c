...
PAYLOAD_CODE int shellcore_fpkg_patch(void)
{
	...
	if (ret)
	{
	 ...
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + enable_patch1), 3, "\x31\xC0\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }

	 ret = proc_write_mem(ssc, (void *)(text_seg_base + enable_patch2), 3, "\x31\xC0\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }

	 ret = proc_write_mem(ssc, (void *)(text_seg_base + enable_patch3), 3, "\x31\xC0\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }

	 ret = proc_write_mem(ssc, (void *)(text_seg_base + enable_patch4), 3, "\x31\xC0\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }

	 ret = proc_write_mem(ssc, (void *)(text_seg_base + enable_patch5), 3, "\x31\xC0\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sceSblACMgrIsAllowedSystemLevelDebugging1), 6, "\xB8\x01\x00\x00\x00\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sceSblACMgrIsAllowedSystemLevelDebugging2), 6, "\xB8\x01\x00\x00\x00\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sceSblACMgrIsAllowedSystemLevelDebugging3), 6, "\xB8\x01\x00\x00\x00\xC3", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 ...
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + sys_rwx_patch), 6, "\x90\x90\x90\x90\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
	 ...
	 ret = proc_write_mem(ssc, (void *)(text_seg_base + map_self3), 6, "\x31\xC0\x90\x90\x90", &n);
	 if (ret)
	 {
		 goto error;
	 }
